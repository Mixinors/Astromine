...
public class FluidInserterBlock extends DefaultedHorizontalFacingBlockWithEntity implements NetworkMember {
		...
		if (!world.isClient && !(player.getStackInHand(hand).getItem() instanceof BucketItem)) {
			player.openHandledScreen(new ExtendedScreenHandlerFactory() {
				...
				public Text getDisplayName() {
					return new TranslatableText("block.astromine.fluid_inserter");
				}

				@Override
				public ScreenHandler createMenu(int syncId, PlayerInventory playerInventory, PlayerEntity player) {
					return new FluidInserterScreenHandler(syncId, playerInventory, blockPos);
				}
			});
