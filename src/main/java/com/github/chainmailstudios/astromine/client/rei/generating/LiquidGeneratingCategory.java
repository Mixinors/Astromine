...
public class LiquidGeneratingCategory extends AbstractEnergyGeneratingCategory<LiquidGeneratingDisplay> {
	...
	public EntryStack getLogo() {
		return EntryStack.create(AstromineBlocks.LIQUID_GENERATOR);
	}
	...
	public List<Widget> setupDisplay(LiquidGeneratingDisplay recipeDisplay, Rectangle bounds) {
		...
		widgets.addAll(AstromineREIPlugin.createFluidDisplay(new Rectangle(innerBounds.getX() + 24, innerBounds.getCenterY() - 28, 12, 48),
				EntryStack.create(recipeDisplay.getFluid()), 5000));
		widgets.add(Widgets.createArrow(new Point(innerBounds.getX() + 44, innerBounds.getY() + 22)));
		return widgets;
	}
}
